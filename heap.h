...
public:
    ...
    void insert(int vertex, int key) {
        if (size == capacity) { // EDGE CASE, terminate program if heap is full.
            return;
        }
        size++;
        int i = size - 1; 
...
private:
    ...
    void minHeapify(int idx){
        ...
        if (smallest != idx) { // bubbling down smallest node
            std::swap(position[heapArray[idx]], position[heapArray[smallest] ]); // swap smallest with curr index (positions).
